...
app.post('/get-user-posts', async (req, res) => {
  let id = await User.getId(req.body.username),
    _posts = await db.query(
      'SELECT posts.post_id, posts.user, users.username, users.firstname, users.surname, posts.description, posts.imgSrc, posts.filter, posts.location, posts.type, posts.sentiment, posts.post_time FROM posts, users WHERE posts.user=? AND posts.user = users.id AND posts.type=? ORDER BY posts.post_time DESC',
      [id, 'user']
    ),
    posts = []
...
app.post('/get-tagged-posts', async (req, res) => {
  let _posts = await db.query(
      'SELECT posts.post_id, posts.user, users.username, users.firstname, users.surname, posts.description, posts.imgSrc, posts.filter, posts.location, posts.type, posts.group_id, posts.sentiment, posts.post_time FROM post_tags, posts, users WHERE post_tags.user = ? AND post_tags.post_id = posts.post_id AND posts.user = users.id ORDER BY posts.post_time DESC',
      [req.body.user]
    ),
    posts = []
...
// TRENDING INSIGHTS — top 5 by combined likes + comments (no schema change required)
app.post('/get-trending-posts', async (req, res) => {
  try {
    let _posts = await db.query(
      `SELECT
        posts.post_id,
...
app.post('/get-post', async (req, res) => {
  let { post_id } = req.body,
    _post = await db.query(
      'SELECT posts.post_id, posts.user, users.username, users.firstname, users.surname, posts.description, posts.imgSrc, posts.filter, posts.location, posts.type, posts.group_id, posts.sentiment, posts.post_time FROM posts, users WHERE posts.post_id = ? AND posts.user = users.id',
      [post_id]
    ),
    {
      tags_count,
