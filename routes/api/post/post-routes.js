...
// POST [REQ = DESC, FILTER, LOCATION, TYPE, GROUP, SENTIMENT, IMAGE(FILE) ]
app.post('/post-it', upload.single('image'), async (req, res) => {
  try {
    let { id } = req.session,
      { desc, filter, location, type, group, sentiment } = req.body,
      filename = `finedge_${new Date().getTime()}.jpg`,
      obj = {
        srcFile: req.file.path,
        destFile: `${root}/dist/posts/${filename}`,
      },
      // Only store valid sentiment values; default to NULL for backward compat
      validSentiment = ['bullish', 'bearish', 'neutral'].includes(sentiment)
        ? sentiment
        : null,
      insert = {
        user: id,
...
// EDIT POST [REQ = POST, DESCRIPTION, SENTIMENT]
app.post('/edit-post', async (req, res) => {
  try {
    let { post_id, description, sentiment } = req.body
    let { id } = req.session
    let validSentiment = ['bullish', 'bearish', 'neutral'].includes(sentiment)
      ? sentiment
      : null

    await db.query(
      'UPDATE posts SET description=?, sentiment=? WHERE post_id=?',
      [description, validSentiment, post_id]
    )
