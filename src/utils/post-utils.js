...
export const addPost = async options => {
  ...
  // sentiment is optional — backend validates and defaults to NULL
  if (sentiment) form.append('sentiment', sentiment)

  let {
    data: { success, mssg, post_id, firstname, surname, filename, sentiment: savedSentiment },
 ...
 * @param {String|null} options.sentiment
 * @param {Function} options.dispatch
 * @param {Function} options.done
 * @param {Function} options.failed
 */
export const editPost = async options => {
  let { post_id, description, sentiment, dispatch, done, failed } = options
  let {
    data: { success, mssg, sentiment: savedSentiment },
  } = await post('/api/edit-post', { description, post_id, sentiment: sentiment || null })

  if (success) {
    dispatch(PostActions.editPost({ post_id, description, sentiment: savedSentiment !== undefined ? savedSentiment : null }))
    done()
  } else {
    failed()
  }
