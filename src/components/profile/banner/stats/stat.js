...
const BannerStat = ({ username, disabled, statType, statValue }) => {
  let displayType = statType
  if (statType === 'favourites') displayType = 'watchlist'
  if (statType === 'recommendations') displayType = 'endorsements'

  let body = (
    <Fragment>
      <span className="pro_hg">{statValue}</span>
      <span className="pro_nhg">{c_first(displayType)}</span>
    </Fragment>
  )
