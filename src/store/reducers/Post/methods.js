...
export const editPost = (posts, { post_id, description, sentiment }) => {
  return posts.map(p => {
    if (p.post_id == post_id) {
      p.description = description
      // Update sentiment in local Redux state; undefined means no change (backward compat)
      if (sentiment !== undefined) p.sentiment = sentiment
    }
    return p
  })
}
